...
class SimEnv3Joints():
    ...
    def jointPositions(self, theta_):
        # calculate positions of each joint for plot
        T = np.eye(self.dofArm)
    ...
    def step(self, q_action):
        ...
        q = np.zeros((q_action.shape[0], 2 * self.numJoints))
        q[0, :] = self.initState
        ...
        pos = np.zeros((q_action.shape[0], 2))
        vel = np.zeros((q_action.shape[0], 2))
        pos[0, :] = self.fKinematics(q[0, ::2])

        u = np.zeros((q_action.shape[0], self.numJoints))

        for i in range(q_action.shape[0] - 1):
            u[i, :] = np.dot(mat_state_, (q_action[i, :] - q[i, :]).T)
    ...
    def sample_action(self):
        q_action = np.random.random([self.dofArm])
    ...
    def reset_goal(self):
        ...
        rand_target = np.random.rand(1,2)*10

    '''
        def policyRepresentationDMP(self, dmp_w):
            # param set up using Table 1 in Schaal's paper
    ...
    # the following two functions useless
    def movie_init(self):
        global line
        print('movie init')
