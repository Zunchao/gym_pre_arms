...
class ActorNetwork(object):
    def __init__(self, sess, state_dim, action_dim, action_bound):
        ...
        with tf.variable_scope('Actor'):
            ...
            self.scaled_a_, self.t_state_in = self._build_net(scope='target_net', trainable=False)
    ...
    def predict_target(self, s_):
        return self.sess.run(self.scaled_a_, feed_dict={self.t_state_in: s_})

class CriticNetwork(object):
    def __init__(self, sess, state_dim, action_dim, gamma):
        self.sess = sess
    ...
    def learn(self, s, a, tq):
        self.sess.run([self.q, self.train_op], feed_dict={self.state_input: s, self.action_input: a, self.target_q: tq})

    def update_target_network(self):
        self.sess.run(self.update_network_c)

    def optm(self, s, a, tq):
        return self.sess.run(self.train_op, feed_dict={self.state_input: s, self.action_input: a, self.target_q: tq})
    ...
    def predict_target(self, s_, a_):
        self.sess.run(self.q_, feed_dict={self.t_state_input: s_, self.t_action_input: a_})
...
if __name__ == '__main__':
    ...
    #print(action_bound,state_dim, action_dim)
    ...
    critic = CriticNetwork(sess, state_dim, action_dim, GAMMA)
