...
class Pend2dBallThrowDMP():
    """
    Simulates the 2-link 2d pendulum using DMPs for the lower level policy and returns rewards based
    on how they are modelled (can be specified in getReward(self, theta))
    """
    def __init__(self):
        self.numBasis = 5
    ...
    def getDesiredTrajDMP(self, dmp_w):
        ...
        for i in range(self.numTrajSteps-1):
            ...
            #print('psi',dmp_w.T, psi,np.dot(dmp_w.T, psi))
            f = np.dot(dmp_w.T, psi) * x[i] / np.sum(psi)
            print('f',f, x[i])
            acc_desired = (alpha * (beta * ( g - q[i,::2] ) - ( q[i,1::2] / tau ) ) + f.T ) * tau**2
            q[i+1,1::2] = q[i,1::2] + acc_desired * self.dt  # theta dot?
    ...
    def transitionFunction(self, x, action):
        """
        Basic Euler integration; but more sophisticated integration methods can be used
        """
    ...
    def simulateSystem(self, des_q):
            ...
            b[0,:] = self.getForwardKinematics(q[0,::2])
            print('dsha',q[0,::2])
            ...
            for i in range(des_q.shape[0]-1):
                ...
                if i > self.releaseStep:
                    ...
                    print('sdgfsg',self.getJacobian(q[i+1,::2])[0])
            print('qubbd : ', [q, u, b, bd])

            return [q, u, b, bd]


    def getReward (self, theta):
        # print('shape rtt ', theta, np.reshape(theta,(-1,self.numBasis)))
        ...
        distFactor = -1e4
        print('data teŕajectory: ',data_traj)
