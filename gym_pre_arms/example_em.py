...
class EM():
    ...
    def calculate_R_and_theta(self, Mu_w, Sigma_w):
        ...
        #print('r theta', R.shape, theta.shape)
        return R, theta
    ...
    def update_omega(self, w, theta):
        # calculate mean
        Mu_w = 0
        #print('first sig7: ', theta)
        #print('first sig6: ', w.shape, theta[:,1])
        for l in range(0, self.numSamples):
            Mu_w += w[l]*theta[:,l]
        #print('first sig5: ', Mu_w.shape)
        Mu_w /= np.sum(w)
        # calculate covariance
        Sigma_w = 0
        for l in range(0, self.numSamples):
            Sigma_w += w[l]*(np.outer((theta[:,l] - Mu_w),(theta[:,l] - Mu_w)))
        #print('first sig4: ', Sigma_w.shape)
        Sigma_w /= np.sum(w)
        #print('first sig3: ', Mu_w.shape, Sigma_w.shape)
